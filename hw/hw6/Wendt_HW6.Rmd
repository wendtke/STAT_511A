...
```{r corn_bar}
corn_data %>%
  select(Variety, Mean, SE) %>% 
  ggplot(aes(x = Variety, y = Mean)) + 
  geom_col(alpha = 0.25, position = "dodge") +
...
```{r corn_em_pairs}
corn_em_pairs <- tidy(pairs(corn_em_info), adjust = "none")
corn_em_pairs
```
...
```{r corn_cld, messages = FALSE}
# library(multcomp)
# corn_cld_info <- glht(corn_lm)
# cld(corn_lm)
```
